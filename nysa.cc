...
static bool get_gate_value(gate_t gate, vector<bool> & inputs) {
    switch (gate) {
        ...
        case AND : return all_of(inputs.begin(), inputs.end(), [](bool i){return i;});
        case NAND : return !all_of(inputs.begin(), inputs.end(), [](bool i){return i;});
        case OR: return any_of(inputs.begin(), inputs.end(), [](bool i){return i;});
        case NOR: return !any_of(inputs.begin(), inputs.end(), [](bool i){return i;});
...
static void
report_signal_multiple_outputs_error(size_t line_nr, int signal_nr) {
...
static void
add_gate(graph_t &graph, gate_map_t &gate_map, set<int> &all_signals,
         set<int> &gate_signals, string &line, size_t line_nr) {
    ...
    vector<int> kids_numbers = convert_to_int_vector(
    ...
    } else report_signal_multiple_outputs_error(line_nr, gate_number);
}
...
static tuple<graph_t, gate_map_t, vector<int> > get_input() {
    ...
    set_difference(all_signals.begin(), all_signals.end(),
                   gate_signals.begin(), gate_signals.end(),
                   back_inserter(input_signals));
